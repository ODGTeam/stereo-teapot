...
int Engine::InitDisplay() {
	...
	if (renderer_.stereo_cam.stereo == STEREO) {
		renderer_.stereo_cam.screenwidth = gl_context_->GetScreenWidth();
		renderer_.stereo_cam.screenheight = gl_context_->GetScreenHeight();
		renderer_.UpdateViewportLeft();
		renderer_.UpdateViewportRight();
	} else {
...
void Engine::DrawFrame() {
	...
	if (renderer_.stereo_cam.stereo == STEREO) {
		renderer_.UpdateViewportLeft();
		renderer_.UpdateLeft(monitor_.GetCurrentTime());
		renderer_.Render(renderer_.mat_projection_left_,
				renderer_.mat_view_left_);

		renderer_.UpdateViewportRight();
		renderer_.UpdateRight(monitor_.GetCurrentTime());
		renderer_.Render(renderer_.mat_projection_right_,
				renderer_.mat_view_right_);
	} else {
		renderer_.Update(monitor_.GetCurrentTime());
		renderer_.Render(renderer_.mat_projection_, renderer_.mat_view_);
	}
...
void android_main(android_app* state) {
	...
	//call to osterhout ext to put windows into extend mode
	//extend_display(state, true);
